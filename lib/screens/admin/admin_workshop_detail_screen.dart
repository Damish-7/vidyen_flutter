...
class _AdminWorkshopDetailScreenState
  ...
  Future<void> _loadReviewers() async {
    try {
      ...
      setState(() {
        _reviewers = all
            .where((r) =>
                (r['review_type'] ?? '').toString().toLowerCase().contains('workshop'))
            .toList();
  ...
  Future<void> _assignReviewer(String reviewerCode) async {
    ...
    try {
      await _service.adminAssignWorkshopReviewer(widget.workshopId, reviewerCode);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    final status = _workshop?['status']?.toString() ?? '';
    return Scaffold(
      appBar: AppBar(
      ...
      bottomNavigationBar: (!_loading && _workshop != null && status == '0')
          ? SafeArea(
              child: Padding(
